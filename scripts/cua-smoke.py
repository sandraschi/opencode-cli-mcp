#!/usr/bin/env python3
"""CUA smoke test for NSIS-installed fleet apps (opencode-cli-mcp).
...
BACKEND_PORT = int(cfg("backend_port", 10951))
BACKEND_URL = f"http://127.0.0.1:{BACKEND_PORT}"
PRODUCT_NAME = cfg("product_name", "Opencode Cli MCP")
...
WINDOW_TITLE_RE = cfg("window_title_re", "Opencode Cli MCP")
BRIDGE_OK_TEXT = cfg("bridge_ok_text", "REST bridge reachable")
INSTALL_DIR = cfg("install_dir", "%LOCALAPPDATA%\\Opencode Cli MCP")
OPERATOR_EXE = cfg("operator_exe", "opencode-cli-mcp-native.exe")
PROCESS_NAMES = cfg("backend_process_names", ["opencode-cli-mcp-native", "opencode-cli-mcp-backend"])
NSIS_GLOB = cfg("nsis_glob", "native/target/release/bundle/nsis/Opencode Cli MCP_*_x64-setup.exe")
REGISTRY_FILTER = cfg("uninstall_registry_filter", "*Opencode Cli MCP*")
MAX_RETRY = 10
RETRY_DELAY = 3
...
def analyze_logs():
    ...
    log_paths = [
        os.path.join(INSTALL_DIR, "opencode-cli-mcp.log"),
        os.path.expandvars(r"%APPDATA%\com.sandraschi.opencode-cli-mcp\logs\backend-spawn.log"),
    ]
    errors = []
    warnings = []
