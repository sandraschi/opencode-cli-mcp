...
export function Chat() {
  ...
  const refreshStatus = () => {
    api.getOllamaStatus().then((d) => setBackendOk(d.data.running)).catch(() => setBackendOk(false));
    api.getSettings().then((s) => setSettings(s as SettingsData)).catch(() => {});
  };
  ...
  const handleRefine = async () => {
    ...
    try {
      const res = await fetch(API_BASE + "/api/chat", {
        ...
        body: JSON.stringify({ message: `Rewrite this prompt to be more clear and specific. Return ONLY the rewritten prompt, no explanation:\n\n${input}`, system: "You are a prompt refiner. Rewrite the user's input to be clearer and more specific. Return only the rewritten text." }),
      });
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) => {
        const updated = [...prev];
        updated[updated.length - 1] = { ...updated[updated.length - 1], content: data.response || data.error || "(no response)" };
        return updated;
      });
    } catch {
      setMessages((prev) => {
        const updated = [...prev];
        updated[updated.length - 1] = { ...updated[updated.length - 1], content: "Error: Could not reach LLM provider. Check Settings." };
        return updated;
      });
    } finally {
      setStreaming(false);
  ...
  const handleExport = useCallback(() => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = `opencode-cli-chat-${new Date().toISOString().slice(0, 10)}.txt`;
    ...
    <div data-testid="chat-page" className="flex flex-col h-full max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-mono bg-zinc-800 px-2 py-0.5 rounded">skill:opencode-cli</span>
          <select data-testid="personality-select" value={persona} onChange={(e) => setPersona(e.target.value)} className="bg-zinc-800 text-xs text-zinc-300 border border-zinc-700 rounded px-2 py-1">
            {PERSONAS.map((p) => <option key={p.id} value={p.id}>{p.label}</option>)}
          </select>
          <div className="flex gap-1">
            <button data-testid="chat-export" onClick={handleExport} disabled={messages.length === 0} className="p-1.5 rounded text-zinc-500 hover:text-zinc-300 disabled:opacity-30" title="Export"><Download className="w-3.5 h-3.5" /></button>
            <button data-testid="chat-clear" onClick={handleClear} disabled={messages.length === 0} className="p-1.5 rounded text-zinc-500 hover:text-zinc-300 disabled:opacity-30" title="Clear"><Eraser className="w-3.5 h-3.5" /></button>
          </div>
          {backendOk === true && (
            <span className="flex items-center gap-1 text-xs text-green-400 bg-green-500/10 px-2 py-1 rounded-full">
              <Wifi className="w-3 h-3" />{providerLabel()}
            </span>
          )}
          {backendOk === false && (
            <span className="flex items-center gap-1 text-xs text-zinc-500 bg-zinc-800 px-2 py-1 rounded-full">
              <WifiOff className="w-3 h-3" />No LLM detected
            </span>
          )}
          {backendOk === null && (
            <span className="flex items-center gap-1 text-xs text-zinc-500 bg-zinc-800 px-2 py-1 rounded-full">
      ...
      <div className="flex gap-2 mb-4">
        ...
        <button onClick={() => setProvider(provider === "local" ? "cloud" : "local")} className={`flex items-center gap-1 px-3 py-1.5 text-xs rounded-lg border transition-colors ${provider === "local" ? "border-green-500/30 text-green-400" : "border-blue-500/30 text-blue-400"}`}>
      ...
      <div data-testid="example-prompts" className="flex flex-wrap gap-1.5 mb-3">
        {EXAMPLE_PROMPTS.map((p) => (
          <button key={p} onClick={() => setInput(p)} className="flex items-center gap-1 px-2 py-1 rounded-full text-[10px] border border-zinc-700 text-zinc-400 hover:text-zinc-200 hover:border-accent/40 transition-colors bg-zinc-900/50">
        ...
        <AnimatePresence>
          {messages.map((msg, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className={`flex gap-3 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              {msg.role === "assistant" && <div className="p-1.5 bg-accent/10 rounded-lg h-fit mt-1"><Bot className="w-4 h-4 text-accent" /></div>}
              <div className={`max-w-[70%] rounded-xl px-4 py-2.5 text-sm ${msg.role === "user" ? "bg-accent text-white" : "bg-surface-light border border-surface-border"}`}>
                {msg.content || <span className="text-zinc-500 italic">Streaming...</span>}
              </div>
              {msg.role === "user" && <div className="p-1.5 bg-zinc-700 rounded-lg h-fit mt-1"><User className="w-4 h-4 text-zinc-300" /></div>}
            </motion.div>
          ))}
        </AnimatePresence>
        <div ref={bottomRef} />
      ...
      <div className="flex-shrink-0">
        <AnimatePresence>
          {refined && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} className="mb-2 p-3 bg-zinc-800/50 border border-zinc-700 rounded-lg">
              <div className="flex items-center gap-1 text-xs text-zinc-400 mb-1"><Sparkles className="w-3 h-3" />Refined prompt</div>
        ...
        <div className="flex gap-2">
          <button onClick={handleRefine} disabled={!input.trim()} className="p-2.5 bg-zinc-800 hover:bg-zinc-700 rounded-lg transition-colors disabled:opacity-30" title="Refine prompt" aria-label="Refine prompt">
