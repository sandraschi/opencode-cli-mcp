...
app = FastAPI(title="OpenCode CLI MCP", lifespan=lifespan)
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

@app.get("/health")
@app.get("/api/health")
async def health(): return {"status": "ok", "server": "OpenCode CLI MCP", "version": "0.1.0"}
...
@app.post("/api/mcpb/install")
async def mcpb_install(body: dict):
    """Install an .mcpb bundle into opencode config. Body: {source: str, name_override?: str, dry_run?: bool}"""
    try:
        from opencode_cli_mcp.tools.mcpb_install import opencode_mcpb_install
        result = await opencode_mcpb_install(
